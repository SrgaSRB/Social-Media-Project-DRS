...
# Postavi putanju do fajla sa korisnicima
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
USERS_FILE = os.path.join(BASE_DIR, '../static/users.json')
...
# Helper funkcija za čuvanje korisnika
...
def search_users(query):
    ...
    return [
        ...
        if query_lower in user['username'].lower() or
           ...
           query_lower in user['firstName'].lower() or
           query_lower in user['lastName'].lower() or
           query_lower in user['address'].lower() or
...
def add_user():
    ...
    # Proveri jedinstvenost korisničkog imena i email-a
